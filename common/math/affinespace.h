...
#define ScalarT typename L::Vector::Scalar
	...
	struct AffineSpaceT
	{
		L l;           /*< linear part of affine space, i.e. linear components */
		VectorT p;     /*< affine part of affine space, i.e. the origin point */
	...
	template<int I = 12>
	struct ArrayXf : final {
		ArrayXf() = delete;
		ArrayXf(const ArrayXf &) = delete;
		ArrayXf& operator = (const ArrayXf &) = delete;
	};
	...
	public:
		...
		static __forceinline ArrayXf<12> copyToArray(const AffineSpace3f& xfm)
		{
			ArrayXf<12> values;
		...
		static __forceinline AffineSpace3f copyFromArray(const float* v)
		{
	...
	public:
		...
		static __forceinline ArrayXf<16> copyToArray(const AffineSpace3f& xfm)
		{
			ArrayXf<16> values;
			values[0] = xfm.l.vx.x;  values[1] = xfm.l.vx.y;  values[2] = xfm.l.vx.z;  values[3] = 0.f;
			values[4] = xfm.l.vy.x;  values[5] = xfm.l.vy.y;  values[6] = xfm.l.vy.z;  values[7] = 0.f;
			values[8] = xfm.l.vz.x;  values[9] = xfm.l.vz.y;  values[10] = xfm.l.vz.z; values[11] = 0.f;
			values[12] = xfm.p.x;    values[13] = xfm.p.y;    values[14] = xfm.p.z;	   values[15] = 1.f;
			return values;
		}
		...
		{
			return AffineSpace3f(LinearSpace3f(Vector3f(v[0], v[1], v[2]),
				Vector3f(v[4], v[5], v[6]),
				Vector3f(v[8], v[9], v[10])),
				Vector3f(v[12], v[13], v[14]));
		}
	};

	using Array12f = ArrayXf<12>;
	using Array16f = ArrayXf<16>;

	__forceinline Array12f copyToArray(const AffineSpace3f& xfm) {
		return Array12f::copyToArray(xfm);
	}

	__forceinline AffineSpace3f copyFromArray(const float* v) {
		return Array12f::copyFromArray(v);
	}
