...
namespace embree
{
	...
	template<typename T> __forceinline T smoothstep(const T edge0, const T edge1, T x)
	{
	...
	template<typename T> __forceinline T smootherstep(const T edge0, const T edge1, T x) {
	...
	template<typename T> __forceinline T lerp(const T v0, const T v1, const T t) {
		return (1 - t) * v0 + t * v1;
	}
