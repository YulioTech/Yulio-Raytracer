...
namespace embree
{
	...
	Ref<Image> loadJPEG(const FileName &filename)
	{
		...
		if (!file) {
			throw std::runtime_error("Unable to open \"" + filename.str() + "\".");
		}
		std::streamsize size = file.tellg();

		file.seekg(0, std::ios::beg);

		std::vector<uint8_t> compressedImage(size);
		if (!file.read(compressedImage.data(), size)) {
			throw std::runtime_error("Unable to read \"" + filename.str() + "\".");
		}
		...
		tjDecompressHeader2(jpegDecompressor, &compressedImage[0], size, &width, &height, &jpegSubsamp);

		if (width <= 0 || height <= 0) {
			throw std::runtime_error("Improper dimensions for the image in \"" + filename.str() + "\".");
		}

		std::vector<uint8_t> decompressedRGB(width*height*3); //!< will contain the decompressed image

		tjDecompress2(jpegDecompressor, &compressedImage[0], size, &decompressedRGB[0], width, 0/*pitch*/, height, TJPF_RGB, TJFLAG_ACCURATEDCT);
		...
		static const float rcp255 = rcp(255.f);
		for (size_t y = 0, yFlip = height - 1, i = 0; y < height; y++, yFlip--) {
			for (size_t x = 0; x < width; x++) {
				const float r = (float)decompressedRGB[i++] * rcp255;
				const float g = (float)decompressedRGB[i++] * rcp255;
				const float b = (float)decompressedRGB[i++] * rcp255;
				image->set(x, yFlip, Color4(r, g, b, 1.f));
			}
		}

		return image;
	}
#else Standard libjpeg API
	...
	Ref<Image> loadJPEG(const FileName &filename)
	{
		...
		if (!file) throw std::runtime_error("Unable to open \"" + filename.str() + "\".");
		...
		for (size_t y = 0, yFlip = height - 1, i = 0; y < height; y++, yFlip--) {
			for (size_t x = 0; x < width; x++) {
				const float r = (float)rgb[i++] * rcp255;
				const float g = (float)rgb[i++] * rcp255;
				const float b = (float)rgb[i++] * rcp255;
				image->set(x, yFlip, Color4(r, g, b, 1.f));
			}
		}
	...
	void storeJPEG(const Ref<Image> &image, const FileName &filename, int quality)
	{
		...
		std::vector<unsigned char> rgb(image->height * image->width * cinfo.input_components, 0U);

		/*! Convert the image to unsigned char RGB. */
		for (size_t y = 0, i = 0; y < image->height; y++) {
			for (size_t x = 0; x < image->width; x++) {
				const Color4 pixel = image->get(x, y);
				rgb[i++] = (unsigned char)(clamp(pixel.r) * 255.f);
				rgb[i++] = (unsigned char)(clamp(pixel.g) * 255.f);
				rgb[i++] = (unsigned char)(clamp(pixel.b) * 255.f);
			}
		}

		/*! Compress and write the image into the target file. */
		compress(&cinfo, &rgb[0]);

		/*! At this point 'jerror.num_warnings' could be checked for corrupt-data warnings. */
		jpeg_destroy_compress(&cinfo); fclose(file);
