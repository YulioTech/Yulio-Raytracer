...
namespace embree
{
	...
	private:
		__forceinline bool stopRequested() { return (stopFlag && (*stopFlag)); }
		std::atomic<bool> *stopFlag = nullptr;

		RendererStatusCallback *statusCallback = nullptr;
		RendererStatus currentStatus;
		__forceinline bool updateStatus(RendererState state) {
			if (!statusCallback) return false;
		...
		__forceinline bool updateStatus(float progress) {
			if (!statusCallback
				|| currentStatus.state != Rendering)
				return false;
