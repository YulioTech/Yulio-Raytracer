...
namespace embree
{
	...
	public:
		...
		PrimitiveHandle(const Ref<InstanceHandle<Shape>>& shape,
			...
			: bstatic(false), faceCamera(faceCamera), shape(shape), light(null), material(material), transform(transform),
		...
		PrimitiveHandle(const Ref<InstanceHandle<Light>>& light,
			const Ref<InstanceHandle<Material>>& material,
			const AffineSpace3f& transform)
			: bstatic(false), faceCamera(false), shape(null), light(light), material(material), transform(transform),
			illumMask(-1), shadowMask(-1) { }

		/*! Constructs new primitive. */
		PrimitiveHandle(const AffineSpace3f& transform, const Ref<PrimitiveHandle>& other)
			: bstatic(other->bstatic), faceCamera(other->faceCamera), shape(other->shape), light(other->light), material(other->material),
		...
		void set(const std::string& property, const Variant& data)
		{
			...
			else if (property == "faceCamera") { faceCamera = data.getBool(); }
		}

		void get(const std::string& property, Variant& data) {
			if (property == "illumMask") { data = illumMask; }
			else if (property == "shadowMask") { data = shadowMask; }
			else if (property == "static") { data = bstatic; }
			else if (property == "faceCamera") { data = faceCamera; }
		}
