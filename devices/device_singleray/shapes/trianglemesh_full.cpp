...
namespace embree
{
	...
	void intersectionFilter(void *ptr, Ray &ray) {
		auto *mesh = (TriangleMeshFull *)ptr;
		const auto &tri = mesh->triangles[ray.id1];

		Vector3f p0 = mesh->position[tri.v0], p1 = mesh->position[tri.v1], p2 = mesh->position[tri.v2];
		if (mesh->unlikely(motion.size())) {
			p0 += ray.time * mesh->motion[tri.v0];
			p1 += ray.time * mesh->motion[tri.v1];
			p2 += ray.time * mesh->motion[tri.v2];
		}
		
		const Vector3f dPdu = p0 - p1, dPdv = p2 - p0;
	...
	BBox3f TriangleMeshFull::extract(RTCScene scene, size_t id) const
	{
		...
		if (cullBackFaces) {
			rtcSetIntersectionFilterFunction(scene, mesh, (RTCFilterFunc)&intersectionFilter);
			rtcSetOcclusionFilterFunction(scene, mesh, (RTCFilterFunc)&occlusionFilter);
