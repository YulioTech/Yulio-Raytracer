...
namespace embree
{
	...
	*/
	...
	public:
		...
		light_mask_t illumMask;    //!< bit mask which light we're interested in (which light masks we receive illum from)
		light_mask_t shadowMask;   //!< bit mask which light we're interested in (which light masks we cast shadows to)
