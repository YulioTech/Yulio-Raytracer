...
#define FPR_VIEW_CAMERA_PREFIX		"YULIO_FPR_VIEW_"
#define CAMERA_ALIGNED_NODE_PREFIX	"YULIO_CAMERA_ALIGNED_"
	...
	bool DAELoader::initSceneMaterials(const aiScene *scene) {
		...
		for (unsigned int i = 0; i < scene->mNumMaterials; ++i) {
			...
			//aiColor4D diffuseColor((float)rand() / RAND_MAX, (float)rand() / RAND_MAX, (float)rand() / RAND_MAX, 1.f); // For testing
			if (textureFilePath == "") {
	...
	bool DAELoader::initSceneCameras(const aiScene *scene, bool toeIn) {
		...
		// Lev: a new behavior - if no cameras tagged with FPR_VIEW_CAMERA_PREFIX are found, then consider all of them as tagged for rendering.
		// This is to accommodate any scene outside of our pipeline when the project is open-sourced.
		unsigned int nTaggedCameras = 0;
		for (unsigned int i = 0; i < scene->mNumCameras; ++i) {

			// Check for the the predefined FPR_VIEW_CAMERA_PREFIX prefix
		...
		for (unsigned int i = 0; i < scene->mNumCameras; ++i) {
			...
			// Only consider cameras with the predefined FPR_VIEW_CAMERA_PREFIX prefix (but only if there are any; see the considerations above)
			std::string cameraName = scene->mCameras[i]->mName.C_Str();
			...
			// Create 12 stereoscopic cube map cameras for each FPR viewpoint
			{
				...
				// ToDo: the near & far plane values should be calculated on a per camera basis depending on the position and orientation of the latter.
	...
	void DAELoader::initSceneMeshesRecursive(const struct aiScene *aiScene, const struct aiNode* node, const aiMatrix4x4 &mParent) {
		...
		for (unsigned int n = 0; n < node->mNumMeshes; ++n) {
...
#if 0
			// Test code: replace materials for certain geometry instances
			{
			...
			default:
				// Lev: double-sidedness can be defined on either a material or a mesh level.
