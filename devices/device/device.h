	...
	*******************************************************************/
	...
	public:
		...
		*******************************************************************/
		 ...
		 *  "Mirror", "Metal", "MetallicPaint", "MatteTextured", "Uber",
		 ...
		 *  handle */
		virtual RTPrimitive rtNewShapePrimitive(RTShape shape, RTMaterial material, const float* transform = NULL, bool faceCamera = false) = 0;
		...
		/*! Updates a primitive in the scene. */
		virtual void rtUpdatePrimitive(RTScene scene, size_t slot, RTPrimitive prim, const Vector3f &camPos, const Vector3f &camUp) = 0;
		 ...
		 *  create (e.g. "debug", "pathtracer", "gpt"). \returns renderer handle */
		...
		*******************************************************************/
		...
		/*! Sets an integer parameter of the handle. */
		virtual void rtSetPointer(RTHandle handle, const char* property, void *p) = 0;

		/*! Sets a float parameter of the handle. */
		virtual void rtSetFloat1(RTHandle handle, const char* property, float x) = 0;
		virtual void rtGetFloat1(RTHandle handle, const char* property, float &x) = 0;
		...
		virtual void rtGetFloat3(RTHandle handle, const char* property, float &x, float &y, float &z) = 0;
		...
		virtual void rtGetString(RTHandle handle, const std::string &property, std::string &str) = 0;
		...
		virtual void rtGetTransform(RTHandle handle, const char* property, AffineSpace3f *transform) = 0;
	...
	/*******************************************************************
					Render status reporting definitions
	*******************************************************************/
	enum RendererState {
		Inactive,
		Rendering,
	...
	struct RendererStatus {
		RendererState state = Inactive;
		float progress = 0.f; // A relative progress value in the range [0.0:1.0]
	};

	//typedef void (*RendererStatusCallback)(const RendererStatus &status);
	using RendererStatusCallback = void (const RendererStatus &status);
}
